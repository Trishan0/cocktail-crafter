...
{
    // Raw digital states with INPUT_PULLUP.
    // 0 = sensor output LOW, 1 = sensor output HIGH.
    uint8_t upperState = digitalRead(IR_UPPER_PIN) == HIGH ? 1 : 0;
    uint8_t lowerState = digitalRead(IR_LOWER_PIN) == HIGH ? 1 : 0;

    output.print("IR:UP:");
    output.print(upperState);
    output.print(",LOW:");
    output.println(lowerState);
}
...
void processSerialCommand(String command, Stream& replyPort)
{
    ...
    else if (command == "CHECK_VALVE_STATE" || command == "GET_VALVE_STATE")
    {
        replyPort.print("VALVE_STATE:");
        replyPort.println(valveStateName(savedValveState));
    }
    else if (command == "SET_VALVE_OPEN")
    {
        if (currentState != SystemState::IDLE)
        {
            replyPort.println("BUSY");
        ...
        saveValveState(ValveSavedState::OPEN);
        replyPort.println("VALVE_STATE:OPEN");
    }
    else if (command == "SET_VALVE_CLOSED")
    {
        if (currentState != SystemState::IDLE)
        {
            replyPort.println("BUSY");
        ...
        saveValveState(ValveSavedState::CLOSED);
        replyPort.println("VALVE_STATE:CLOSED");
    }
    else if (command == "SET_VALVE_UNKNOWN")
    {
        if (currentState != SystemState::IDLE)
        {
            replyPort.println("BUSY");
        ...
        saveValveState(ValveSavedState::UNKNOWN);
        replyPort.println("VALVE_STATE:UNKNOWN");
    }
    else if (command == "RESET_VALVE_STATE")
    {
        if (currentState != SystemState::IDLE)
        {
            replyPort.println("BUSY");
        ...
        preferences.remove(VALVE_PREF_KEY);
        savedValveState = ValveSavedState::UNKNOWN;
        replyPort.println("VALVE_STATE:RESET_TO_UNKNOWN");
    }
    else
    {
...
void setup()
{
    ...
    Serial.println("Commands: START (or S), STOP (or X), CHECK_LEVELS, CHECK_IR");
    Serial.println("Valve state: CHECK_VALVE_STATE, SET_VALVE_OPEN, SET_VALVE_CLOSED");
    Serial.println("Valve state reset: SET_VALVE_UNKNOWN, RESET_VALVE_STATE");
    Serial.println("Level response: LEVELS:PB0,PB1,PB2,PB3,PB4,PB5");
    Serial.println("IR response: IR:UP:0,LOW:1");
    Serial.println("Sensor values are raw: 1 = HIGH, 0 = LOW");
